...
CONFIG_TIMER:
	...
	movia	r7, TIM_VAL		# load TIME_VAL into r7
	ldw		r9, (r7)			# load valye of TIME_VAL into r9
...
KEY_ISR:
	/* check key 0 vs key1 vs key2 */
	
	movia 	r7, KEYs 			# store addy of KEYs in r7
	...
	movi 	r10, 1
	andi 	r9, r8, 0x1
	beq 	r9, r10, KEY_ISR_0
	
	# if key 1, store 10 -> 0x2
	movi 	r10, 2
	andi 	r9, r8, 0x2
	beq 	r9, r10, KEY_ISR_1
	
	# if key 2, store 100 -> 0x4
	movi 	r10, 4
	andi 	r9, r8, 0x4
	beq 	r9, r10, KEY_ISR_2
...
KEY_ISR_0:
	...
	br KEY_ISR_END 				# go to KEY_ISR_END to reset edge capture
	
KEY_ISR_1:
	/* if key 1, reset, and increment */
	
	# incrementing TIM_VAL
	movia	r7, TIM_VAL 		# load TIM_VAL addy into r7
	ldw 	r8, (r7)			# load TIM_VAL into r8
	slli 	r8, r8, 1			# shift left TIM_VAL to multiply by 2
	stw 	r8, (r7)			# store back in TIM_VAL
	
	# resetting timer
	br KEY_ISR_RESET
	
KEY_ISR_2:
	/* if key 2, reset and decrement */
	
	# decrementing TIM_VAL
	movia	r7, TIM_VAL 		# load TIM_VAL addy into r7
	ldw 	r8, (r7)			# load TIM_VAL into r8
	srli 	r8, r8, 1			# shift right TIM_VAL to multiply by 2
	stw 	r8, (r7)			# store back in TIM_VAL
	
	br KEY_ISR_RESET 				# go to KEY_ISR_RESET to reset timer
...
KEY_ISR_RESET:	
	...
	stwio 	r13, 0xC(r12)		# store in timer to reset
...
.global TIM_VAL
TIM_VAL: .word 25000000		# used by timer as starting value
.end